...
class TvShowDetailsFragment : BaseFragment() {
    ...
    private fun setupBackdropImageView(tvShowDetails: TvShowDetails) {
        val loadBackDrop = tvShowDetails.backDropPath ?: R.drawable.bg_backdrop_placeholder
    ...
    private fun setupPosterImageView(tvShowDetails: TvShowDetails) {
        val loadPoster = tvShowDetails.posterPath ?: R.drawable.bg_poster_placeholder
    ...
    private fun setupCreatedByViews(tvShowDetails: TvShowDetails) {
        with(binding) {
            ...
            val personSimpleAdapterListener = viewModel.personSimpleAdapterListener

            if (!createdBy.isNullOrEmpty()) {
                val creatorAdapter = CreatorAdapter(createdBy, personSimpleAdapterListener)
    ...
    private fun setupSeasonsViews(tvShowDetails: TvShowDetails) {
        with(binding) {
            ...
            if (!seasons.isNullOrEmpty()) {
                val seasonSimpleListener = viewModel.seasonSimpleListener
                val seasonAdapter = SeasonAdapter(seasons, seasonSimpleListener)
